...
- Run focused tests (`pnpm scripts:test -- --filter <script>`) before shipping, and always cover the worktree bootstrap spec (`--filter create-worktree-pr`) when `pnpm gh:worktree` behavior shifts.
